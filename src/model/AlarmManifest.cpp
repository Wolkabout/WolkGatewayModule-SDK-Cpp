...
namespace wolkabout {
AlarmManifest::AlarmManifest(std::string name,
                             AlarmManifest::AlarmSeverity severity,
                             std::string reference, std::string message,
                             std::string description)
    : m_name(std::move(name)), m_severity(severity),
      m_reference(std::move(reference)), m_message(std::move(message)),
      m_description(std::move(description)) {}

const std::string &AlarmManifest::getName() const { return m_name; }
...
const std::string &AlarmManifest::getReference() const { return m_reference; }
...
const std::string &AlarmManifest::getMessage() const { return m_message; }
...
bool AlarmManifest::operator==(AlarmManifest &rhs) const {
  if (m_name != rhs.m_name || m_severity != rhs.m_severity ||
      m_reference != rhs.m_reference || m_message != rhs.m_message ||
