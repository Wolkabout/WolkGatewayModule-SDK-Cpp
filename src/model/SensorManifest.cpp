...
namespace wolkabout {
SensorManifest::SensorManifest(std::string name, std::string reference,
                               std::string description, std::string unit,
                               std::string readingType,
                               SensorManifest::DataType dataType,
                               unsigned int precision, double minimum,
                               double maximum)
    : m_name(std::move(name)), m_reference(std::move(reference)),
      m_description(std::move(description)), m_unit(std::move(unit)),
      m_readingType(std::move(readingType)), m_dataType(dataType),
      m_precision(precision), m_minimum(minimum), m_maximum(maximum),
      m_delimiter(""), m_labels({}) {}
...
SensorManifest::SensorManifest(std::string name, std::string reference,
                               ...
                               SensorManifest::DataType dataType,
                               unsigned int precision, double minimum,
                               double maximum, std::string delimiter,
                               std::vector<std::string> labels)
    : m_name(std::move(name)), m_reference(std::move(reference)),
      m_description(std::move(description)), m_unit(std::move(unit)),
      m_readingType(std::move(readingType)), m_dataType(dataType),
      m_precision(precision), m_minimum(minimum), m_maximum(maximum),
      m_delimiter(std::move(delimiter)), m_labels(std::move(labels)) {}

const std::string &SensorManifest::getName() const { return m_name; }
...
const std::string &SensorManifest::getReference() const { return m_reference; }
...
const std::string &SensorManifest::getUnit() const { return m_unit; }
...
unsigned int SensorManifest::getPrecision() const { return m_precision; }
...
double SensorManifest::getMinimum() const { return m_minimum; }
...
double SensorManifest::getMaximum() const { return m_maximum; }
...
const std::string &SensorManifest::getDelimiter() const { return m_delimiter; }
...
bool SensorManifest::operator==(SensorManifest &rhs) const {
  if (m_name != rhs.m_name || m_reference != rhs.m_reference ||
      m_description != rhs.m_description || m_unit != rhs.m_unit ||
      m_readingType != rhs.m_readingType || m_dataType != rhs.m_dataType ||
