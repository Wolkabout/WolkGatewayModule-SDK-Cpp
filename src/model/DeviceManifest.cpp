...
wolkabout::DeviceManifest::DeviceManifest(
    std::string name, std::string description, std::string protocol,
    ...
    : m_name(std::move(name)), m_description(std::move(description)),
      ...
      m_sensors(std::move(sensors)), m_alarms(std::move(alarms)),
...
wolkabout::DeviceManifest &wolkabout::DeviceManifest::addSensor(
    const wolkabout::SensorManifest &sensorManifest) {
  m_sensors.push_back(sensorManifest);
  return *this;
}

wolkabout::DeviceManifest &wolkabout::DeviceManifest::addAlarm(
    const wolkabout::AlarmManifest &alarmManifest) {
  m_alarms.push_back(alarmManifest);
  return *this;
}

wolkabout::DeviceManifest &wolkabout::DeviceManifest::addActuator(
    const wolkabout::ActuatorManifest &actuatorManifest) {
  m_actuators.push_back(actuatorManifest);
  return *this;
}

const std::vector<wolkabout::ConfigurationManifest> &
wolkabout::DeviceManifest::getConfigurations() const {
  return m_configurations;
}

const std::vector<wolkabout::SensorManifest> &
wolkabout::DeviceManifest::getSensors() const {
  return m_sensors;
}

const std::vector<wolkabout::AlarmManifest> &
wolkabout::DeviceManifest::getAlarms() const {
  return m_alarms;
}

const std::vector<wolkabout::ActuatorManifest> &
wolkabout::DeviceManifest::getActuators() const {
  return m_actuators;
}

const std::string &wolkabout::DeviceManifest::getName() const { return m_name; }
...
const std::string &
wolkabout::DeviceManifest::getFirmwareUpdateProtocol() const {
  return m_firmwareUpdateProtocol;
}

bool wolkabout::DeviceManifest::operator==(DeviceManifest &rhs) const {
  if (m_name != rhs.m_name || m_description != rhs.m_description ||
