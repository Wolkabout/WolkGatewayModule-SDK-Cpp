...
public:
  ...
  DeviceManifest(std::string name, std::string description,
                 std::string protocol, std::string firmwareUpdateProtocol,
                 std::vector<ConfigurationManifest> configurations = {},
                 std::vector<SensorManifest> sensors = {},
                 std::vector<AlarmManifest> alarms = {},
                 std::vector<ActuatorManifest> actuators = {});

  virtual ~DeviceManifest() = default;

  DeviceManifest &
  addConfiguration(const ConfigurationManifest &configurationManifest);
