...
public:
  enum class Type { FILE_UPLOAD, URL_DOWNLOAD, INSTALL, ABORT, UNKNOWN = -1 };

  FirmwareUpdateCommand();

  explicit FirmwareUpdateCommand(FirmwareUpdateCommand::Type type);

  FirmwareUpdateCommand(FirmwareUpdateCommand::Type type, std::string name,
                        std::uint_fast64_t size, std::string hash,
                        bool autoInstall);

  FirmwareUpdateCommand(FirmwareUpdateCommand::Type type, std::string url,
                        bool autoInstall);
