...
{
    const std::shared_ptr<Message> outboundMessage = OutboundMessageFactory::make(m_device.getKey(), response);
...
{
    const std::shared_ptr<Message> outboundMessage = OutboundMessageFactory::make(m_device.getKey(), request);
