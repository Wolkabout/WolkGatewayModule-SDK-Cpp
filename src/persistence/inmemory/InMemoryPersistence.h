...
public:
  ...
  bool putSensorReading(const std::string &key,
                        std::shared_ptr<SensorReading> sensorReading) override;
  std::vector<std::shared_ptr<SensorReading>>
  getSensorReadings(const std::string &key, std::uint_fast64_t count) override;
  void removeSensorReadings(const std::string &key,
  ...
  getAlarms(const std::string &key, std::uint_fast64_t count) override;
  ...
  putActuatorStatus(const std::string &key,
                    std::shared_ptr<ActuatorStatus> actuatorStatus) override;
  std::shared_ptr<ActuatorStatus>
  getActuatorStatus(const std::string &key) override;
...
private:
  std::vector<std::shared_ptr<SensorReading>> &
  getOrCreateSensorReadingsByKey(const std::string &key);
  std::vector<std::shared_ptr<Alarm>> &
  getOrCreateAlarmsByKey(const std::string &key);
