...
public:
  virtual std::string
  extractReferenceFromChannel(const std::string &topic) const = 0;
  ...
  virtual std::unique_ptr<ActuatorGetCommand>
  makeActuatorGetCommand(std::shared_ptr<Message> message) const = 0;
  virtual std::unique_ptr<ActuatorSetCommand>
  makeActuatorSetCommand(std::shared_ptr<Message> message) const = 0;
  ...
  virtual std::shared_ptr<Message> makeMessage(
      const std::string &deviceKey,
      std::vector<std::shared_ptr<ActuatorStatus>> actuatorStatuses) const = 0;
