...
const std::string JsonProtocol::SENSOR_READING_TOPIC_ROOT =
    "d2p/sensor_reading/";
const std::string JsonProtocol::EVENTS_TOPIC_ROOT = "d2p/events/";
const std::string JsonProtocol::ACTUATION_STATUS_TOPIC_ROOT =
    "d2p/actuator_status/";
const std::string JsonProtocol::CONFIGURATION_SET_RESPONSE_TOPIC_ROOT =
    "d2p/configuration_set/";
const std::string JsonProtocol::CONFIGURATION_GET_RESPONSE_TOPIC_ROOT =
...
const std::string JsonProtocol::CONFIGURATION_SET_REQUEST_TOPIC_ROOT =
    "p2d/configuration_set/";
const std::string JsonProtocol::CONFIGURATION_GET_REQUEST_TOPIC_ROOT =
    "p2d/configuration_get/";

const std::vector<std::string> JsonProtocol::INBOUND_CHANNELS = {
    ACTUATION_GET_TOPIC_ROOT + CHANNEL_WILDCARD,
...
const std::string &JsonProtocol::getName() const { return NAME; }

const std::vector<std::string> &JsonProtocol::getInboundChannels() const {
  return INBOUND_CHANNELS;
}

std::shared_ptr<Message> JsonProtocol::makeMessage(
  ...
  const std::string topic = SENSOR_READING_TOPIC_ROOT + deviceKey +
                            CHANNEL_DELIMITER + REFERENCE_PATH_PREFIX +
...
JsonProtocol::makeMessage(const std::string &deviceKey,
  ...
  const std::string payload = jPayload.dump();
  const std::string topic = EVENTS_TOPIC_ROOT + deviceKey + CHANNEL_DELIMITER +
...
std::shared_ptr<Message> JsonProtocol::makeMessage(
  ...
  const std::string payload = jPayload.dump();
  const std::string topic = ACTUATION_STATUS_TOPIC_ROOT + DEVICE_PATH_PREFIX +
                            deviceKey + CHANNEL_DELIMITER +
                            REFERENCE_PATH_PREFIX +
...
JsonProtocol::makeActuatorSetCommand(std::shared_ptr<Message> message) const {
  try {
    ...
    return std::unique_ptr<ActuatorSetCommand>(
        new ActuatorSetCommand(reference, value));
  } catch (...) {
    LOG(DEBUG) << "Unable to parse ActuatorSetCommand: "
               << message->getContent();
    return nullptr;
  }
}
...
JsonProtocol::makeActuatorGetCommand(std::shared_ptr<Message> message) const {
  try {
    const auto reference = extractReferenceFromChannel(message->getChannel());
    if (reference.empty()) {
      return nullptr;
    ...
    return std::unique_ptr<ActuatorGetCommand>(
        new ActuatorGetCommand(reference));
  } catch (...) {
    LOG(DEBUG) << "Unable to parse ActuatorGetCommand: "
               << message->getContent();
    return nullptr;
  }
}
...
std::string
JsonProtocol::extractReferenceFromChannel(const std::string &topic) const {
  ...
  const std::string referencePathPrefix =
      CHANNEL_DELIMITER + REFERENCE_PATH_PREFIX;
...
std::string
JsonProtocol::extractDeviceKeyFromChannel(const std::string &topic) const {
  LOG(TRACE) << METHOD_INFO;
