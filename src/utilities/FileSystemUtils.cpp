...
bool FileSystemUtils::createFileWithContent(const std::string &filePath,
...
bool FileSystemUtils::createBinaryFileWithContent(const std::string &filePath,
  ...
  try {
    ...
    ofstream.write(reinterpret_cast<const char *>(content.data()),
                   content.size());
    if (!ofstream) {
      deleteFile(filePath);
      return false;
...
bool FileSystemUtils::readFileContent(const std::string &filePath,
                                      std::string &content) {
  ...
  content = std::string((std::istreambuf_iterator<char>(ifstream)),
                        std::istreambuf_iterator<char>());
  return true;
}

bool FileSystemUtils::readBinaryFileContent(const std::string &filePath,
                                            ByteArray &content) {
