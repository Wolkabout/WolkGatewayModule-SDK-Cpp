...
#define INSTANTIATE_ADD_SENSOR_READING_FOR(x)                                  \
  template void Wolk::addSensorReading<x>(const std::string &deviceKey,        \
                                          const std::string &reference,        \
                                          x value, unsigned long long rtc)

namespace wolkabout {
WolkBuilder Wolk::newBuilder() { return WolkBuilder(); }

template <>
void Wolk::addSensorReading(const std::string &deviceKey,
                            const std::string &reference, std::string value,
  ...
  addToCommandBuffer([=]() -> void {
    ...
    if (!sensorDefinedForDevice(deviceKey, reference)) {
      LOG(ERROR) << "Sensor does not exist for device: " << deviceKey << ", "
                 << reference;
      return;
    }
...
template <typename T>
void Wolk::addSensorReading(const std::string &deviceKey,
                            const std::string &reference, T value,
                            unsigned long long rtc) {
...
template <>
void Wolk::addSensorReading(const std::string &deviceKey,
                            const std::string &reference, bool value,
                            unsigned long long rtc) {
  addSensorReading(deviceKey, reference, std::string(value ? "true" : "false"),
                   rtc);
}

template <>
void Wolk::addSensorReading(const std::string &deviceKey,
                            const std::string &reference, char *value,
                            unsigned long long rtc) {
...
template <>
void Wolk::addSensorReading(const std::string &deviceKey,
                            const std::string &reference, const char *value,
                            unsigned long long rtc) {
  addSensorReading(deviceKey, reference, std::string(value), rtc);
}
...
void Wolk::addAlarm(const std::string &deviceKey, const std::string &reference,
                    const std::string &value, unsigned long long rtc) {
  if (rtc == 0) {
    rtc = Wolk::currentRtc();
  }
  ...
  addToCommandBuffer([=]() -> void {
    ...
    if (!alarmDefinedForDevice(deviceKey, reference)) {
      LOG(ERROR) << "Alarm does not exist for device: " << deviceKey << ", "
                 << reference;
      return;
    }
...
void Wolk::publishActuatorStatus(const std::string &deviceKey,
                                 const std::string &reference) {
  ...
  addToCommandBuffer([=]() -> void {
    if (m_connectivityService->connect()) {
      ...
      for (const auto &kvp : m_devices) {
        for (const std::string &actuatorReference :
             kvp.second.getActuatorReferences()) {
...
Wolk::~Wolk() { m_commandBuffer->stop(); }
...
void Wolk::handleActuatorSetCommand(const std::string &key,
                                    const std::string &reference,
  ...
  addToCommandBuffer([=] {
    ...
    if (!actuatorDefinedForDevice(key, reference)) {
      LOG(ERROR) << "Actuator does not exist for device: " << key << ", "
                 << reference;
      return;
    }
    ...
    m_dataService->addActuatorStatus(key, reference, actuatorStatus.getValue(),
                                     actuatorStatus.getState());
    m_dataService->publishActuatorStatuses();
  });
}

void Wolk::handleActuatorGetCommand(const std::string &key,
                                    const std::string &reference) {
  addToCommandBuffer([=] {
    if (!deviceExists(key)) {
      LOG(ERROR) << "Device does not exist: " << key;
...
void Wolk::registerDevice(const Device &device) {
  addToCommandBuffer(
      [=] { m_deviceRegistrationService->publishRegistrationRequest(device); });
}
...
bool Wolk::sensorDefinedForDevice(const std::string &deviceKey,
                                  const std::string &reference) {
  auto it = m_devices.find(deviceKey);
  if (it == m_devices.end()) {
    return false;
  ...
  auto sensorIt = std::find_if(sensors.begin(), sensors.end(),
                               [&](const SensorManifest &manifest) {
                                 return manifest.getReference() == reference;
                               });

  return sensorIt != sensors.end();
}

bool Wolk::alarmDefinedForDevice(const std::string &deviceKey,
                                 const std::string &reference) {
  auto it = m_devices.find(deviceKey);
  if (it == m_devices.end()) {
    return false;
...
bool Wolk::actuatorDefinedForDevice(const std::string &deviceKey,
                                    const std::string &reference) {
  auto it = m_devices.find(deviceKey);
  if (it == m_devices.end()) {
    return false;
...
void Wolk::handleRegistrationResponse(
    const std::string &deviceKey, DeviceRegistrationResponse::Result result) {
  LOG(INFO) << "Registration response for device '" << deviceKey
            << "' received: " << static_cast<int>(result);
}

Wolk::ConnectivityFacade::ConnectivityFacade(
    InboundMessageHandler &handler, std::function<void()> connectionLostHandler)
    : m_messageHandler{handler}, m_connectionLostHandler{
                                     connectionLostHandler} {}

void Wolk::ConnectivityFacade::messageReceived(const std::string &channel,
                                               const std::string &message) {
  m_messageHandler.messageReceived(channel, message);
}

void Wolk::ConnectivityFacade::connectionLost() { m_connectionLostHandler(); }
