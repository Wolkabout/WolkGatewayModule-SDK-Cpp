...
WolkBuilder &WolkBuilder::actuationHandler(
    const std::function<void(const std::string &, const std::string &,
                             const std::string &)> &actuationHandler) {
...
WolkBuilder &WolkBuilder::actuationHandler(
    std::shared_ptr<ActuationHandler> actuationHandler) {
...
WolkBuilder &WolkBuilder::actuatorStatusProvider(
    const std::function<ActuatorStatus(
        const std::string &, const std::string &)> &actuatorStatusProvider) {
...
WolkBuilder &WolkBuilder::actuatorStatusProvider(
    std::shared_ptr<ActuatorStatusProvider> actuatorStatusProvider) {
...
WolkBuilder &WolkBuilder::deviceStatusProvider(
    const std::function<DeviceStatus(const std::string &deviceKey)>
...
WolkBuilder &WolkBuilder::deviceStatusProvider(
    std::shared_ptr<DeviceStatusProvider> deviceStatusProvider) {
...
std::unique_ptr<Wolk> WolkBuilder::build() {
  ...
  wolk->m_connectivityService.reset(new MqttConnectivityService(
      std::make_shared<PahoMqttClient>(), "", "", m_host));

  wolk->m_inboundMessageHandler.reset(new InboundGatewayMessageHandler());

  wolk->m_connectivityManager = std::make_shared<Wolk::ConnectivityFacade>(
  ...
  wolk->m_dataService = std::make_shared<DataService>(
      *wolk->m_dataProtocol, *wolk->m_persistence, *wolk->m_connectivityService,
      ...
      [&](const std::string &key, const std::string &reference) {
        wolk->handleActuatorGetCommand(key, reference);
      });
  ...
  wolk->m_deviceRegistrationService =
      std::make_shared<DeviceRegistrationService>(
...
wolkabout::WolkBuilder::operator std::unique_ptr<Wolk>() { return build(); }

WolkBuilder::WolkBuilder()
    : m_host{MESSAGE_BUS_HOST}, m_persistence{new InMemoryPersistence()},
