...
FileHandler::StatusCode FileHandler::handleData(const BinaryData &binaryData) {
  ...
  m_currentPacketData.insert(m_currentPacketData.end(),
                             binaryData.getData().begin(),
                             binaryData.getData().end());
  m_previousPacketHash = binaryData.getHash();

  return FileHandler::StatusCode::OK;
}

FileHandler::StatusCode
FileHandler::validateFile(const ByteArray &fileHash) const {
...
FileHandler::StatusCode
FileHandler::saveFile(const std::string &filePath) const {
  if (FileSystemUtils::createBinaryFileWithContent(filePath,
                                                   m_currentPacketData)) {
    return FileHandler::StatusCode::OK;
  }
