...
std::shared_ptr<Message> OutboundMessageFactory::make(const std::string& deviceKey,
                                                      std::vector<std::shared_ptr<SensorReading>> sensorReadings)
{
    ...
    const std::string topic = SENSOR_READINGS_TOPIC_ROOT + deviceKey + "/r/" + sensorReadings.front()->getReference();
                                                      ...
                                                      std::vector<std::shared_ptr<Alarm>> alarms)
{
    ...
    const std::string payload = jPayload.dump();
    const std::string topic = ALARMS_TOPIC_ROOT + deviceKey + "/r/" + alarms.front()->getReference();
                                                      ...
                                                      std::vector<std::shared_ptr<ActuatorStatus>> actuatorStatuses)
{
    ...
    const std::string payload = jPayload.dump();
    const std::string topic = ACTUATOR_STATUS_TOPIC_ROOT + deviceKey + "/r/" + actuatorStatuses.front()->getReference();
...
std::shared_ptr<Message> OutboundMessageFactory::makeFromFirmwareVersion(const std::string& deviceKey,
                                                                         const std::string& firmwareVerion)
{
    const std::string topic = FIRMWARE_VERSION_TOPIC_ROOT + deviceKey;
    return std::make_shared<Message>(firmwareVerion, topic);
}
}
