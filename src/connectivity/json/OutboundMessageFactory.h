...
public:
    ...
    static std::shared_ptr<Message> make(const std::string& deviceKey,
                                         std::vector<std::shared_ptr<SensorReading>> sensorReadings);
    static std::shared_ptr<Message> make(const std::string& deviceKey, std::vector<std::shared_ptr<Alarm>> alarms);
    static std::shared_ptr<Message> make(const std::string& deviceKey,
    ...
    static std::shared_ptr<Message> make(const std::string& deviceKey, const FilePacketRequest& filePacketRequest);

    static std::shared_ptr<Message> makeFromFirmwareVersion(const std::string& deviceKey,
                                                            const std::string& firmwareVerion);

private:
    static const constexpr char* SENSOR_READINGS_TOPIC_ROOT = "d2p/sensor_reading/d/";
    static const constexpr char* ALARMS_TOPIC_ROOT = "d2p/events/d/";
    static const constexpr char* ACTUATOR_STATUS_TOPIC_ROOT = "d2p/actuator_status/d/";
