...
MqttConnectivityService::MqttConnectivityService(
    std::shared_ptr<MqttClient> mqttClient, std::string key,
    std::string password, std::string host)
    : m_mqttClient(std::move(mqttClient)), m_key(std::move(key)),
      m_password(std::move(password)), m_host(std::move(host)),
      m_lastWillChannel(""), m_lastWillPayload(""), m_lastWillRetain(false),
      m_connected(false) {
  m_mqttClient->onMessageReceived(
      [this](std::string topic, std::string message) -> void {
...
bool MqttConnectivityService::connect() {
  m_mqttClient->setLastWill(m_lastWillChannel, m_lastWillPayload,
                            m_lastWillRetain);
  bool isConnected =
      m_mqttClient->connect(m_key, m_password, TRUST_STORE, m_host, m_key);
  if (isConnected) {
    if (auto handler = m_listener.lock()) {
...
void MqttConnectivityService::disconnect() { m_mqttClient->disconnect(); }
...
bool MqttConnectivityService::publish(std::shared_ptr<Message> outboundMessage,
                                      bool persistent) {
  return m_mqttClient->publish(outboundMessage->getChannel(),
                               outboundMessage->getContent(), persistent);
}

void MqttConnectivityService::setUncontrolledDisonnectMessage(
    std::shared_ptr<Message> outboundMessage, bool persistent) {
