...
DataService::DataService(DataProtocol &protocol, Persistence &persistence,
                         ConnectivityService &connectivityService,
                         const ActuatorSetHandler &actuatorSetHandler,
                         const ActuatorGetHandler &actuatorGetHandler)
    : m_protocol{protocol}, m_persistence{persistence},
      m_connectivityService{connectivityService},
      m_actuatorSetHandler{actuatorSetHandler}, m_actuatorGetHandler{
                                                    actuatorGetHandler} {}
...
void DataService::messageReceived(std::shared_ptr<Message> message) {
  ...
  if (m_protocol.isActuatorGetMessage(message->getChannel())) {
    ...
    if (!command) {
      LOG(WARN) << "Unable to parse message contents: "
                << message->getContent();
      return;
    }
    ...
    m_actuatorSetHandler(deviceKey, command->getReference(),
                         command->getValue());
  } else {
...
const Protocol &DataService::getProtocol() { return m_protocol; }

void DataService::addSensorReading(const std::string &deviceKey,
                                   const std::string &reference,
                                   const std::string &value,
                                   unsigned long long int rtc) {
  auto sensorReading = std::make_shared<SensorReading>(value, reference, rtc);

  m_persistence.putSensorReading(makePersistenceKey(deviceKey, reference),
                                 sensorReading);
}

void DataService::addAlarm(const std::string &deviceKey,
                           const std::string &reference,
                           const std::string &value,
                           unsigned long long int rtc) {
...
void DataService::addActuatorStatus(const std::string &deviceKey,
                                    const std::string &reference,
                                    const std::string &value,
                                    ActuatorStatus::State state) {
  auto actuatorStatusWithRef =
      std::make_shared<ActuatorStatus>(value, reference, state);

  m_persistence.putActuatorStatus(makePersistenceKey(deviceKey, reference),
                                  actuatorStatusWithRef);
}

void DataService::publishSensorReadings() {
  for (const auto &key : m_persistence.getSensorReadingsKeys()) {
    const auto sensorReadings =
        m_persistence.getSensorReadings(key, PUBLISH_BATCH_ITEMS_COUNT);
    ...
    const std::shared_ptr<Message> outboundMessage =
        m_protocol.makeMessage(pair.first, sensorReadings);
...
void DataService::publishAlarms() {
  for (const auto &key : m_persistence.getAlarmsKeys()) {
    ...
    const std::shared_ptr<Message> outboundMessage =
        m_protocol.makeMessage(pair.first, alarms);
...
void DataService::publishActuatorStatuses() {
  for (const auto &key : m_persistence.getGetActuatorStatusesKeys()) {
    ...
    const std::shared_ptr<Message> outboundMessage =
        m_protocol.makeMessage(pair.first, {actuatorStatus});
...
std::string DataService::makePersistenceKey(const std::string &deviceKey,
                                            const std::string &reference) {
  return deviceKey + PERSISTENCE_KEY_DELIMITER + reference;
}

std::pair<std::string, std::string>
DataService::parsePersistenceKey(const std::string &key) {
  ...
  auto reference =
      key.substr(pos + PERSISTENCE_KEY_DELIMITER.size(), std::string::npos);
