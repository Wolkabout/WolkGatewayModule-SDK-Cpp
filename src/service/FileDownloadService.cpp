...
FileDownloadService::FileDownloadService(
    ...
    : m_maxFileSize{maxFileSize}, m_maxPacketSize{maxPacketSize},
      ...
      m_currentFileName{""}, m_currentFileSize{0}, m_currentPacketSize{0},
      m_currentPacketCount{0}, m_currentPacketIndex{0}, m_currentFileHash{},
      m_currentDownloadDirectory{""}, m_retryCount{0} {
  m_currentState = m_idleState.get();
}

void FileDownloadService::download(
    const std::string &fileName, uint_fast64_t fileSize,
    const ByteArray &fileHash, const std::string &downloadDirectory,
    std::function<void(const std::string &filePath)> onSuccessCallback,
    std::function<void(WolkaboutFileDownloader::Error errorCode)>
        onFailCallback) {
  addToCommandBuffer([=] {
    m_currentState->download(fileName, fileSize, fileHash, downloadDirectory,
...
void FileDownloadService::requestPacket(unsigned index, uint_fast64_t size) {
  ++m_retryCount;
  m_outboundDataHandler->addFilePacketRequest(
      FilePacketRequest{m_currentFileName, index, size});

  m_timer.start(PACKET_REQUEST_TIMEOUT_MSEC, [=] {
    abort();
    if (m_currentOnFailCallback) {
      m_currentOnFailCallback(
          WolkaboutFileDownloader::Error::UNSPECIFIED_ERROR);
    }
  });
}
...
void FileDownloadService::packetFailed() {
  if (m_retryCount == FileDownloadService::MAX_RETRY_COUNT) {
    ...
    if (m_currentOnFailCallback) {
      m_currentOnFailCallback(
          WolkaboutFileDownloader::Error::RETRY_COUNT_EXCEEDED);
    }
  } else {
...
void FileDownloadService::IdleState::download(
  ...
  if (fileSize <=
      m_service.m_maxPacketSize - (2 * ByteUtils::SHA_256_HASH_BYTE_LENGTH)) {
    m_service.m_currentPacketCount = 1;
    m_service.m_currentPacketSize =
        fileSize + (2 * ByteUtils::SHA_256_HASH_BYTE_LENGTH);
  } else {
    const auto count =
...
void FileDownloadService::DownloadState::handleBinaryData(
  ...
  case FileHandler::StatusCode::OK: {
    if (++m_service.m_currentPacketIndex == m_service.m_currentPacketCount) {
      ...
      const auto validationResult =
          m_service.m_fileHandler->validateFile(m_service.m_currentFileHash);
      switch (validationResult) {
      case FileHandler::StatusCode::OK: {
        const std::string filePath = m_service.m_currentDownloadDirectory +
                                     "/" + m_service.m_currentFileName;
        ...
        case FileHandler::StatusCode::FILE_HANDLING_ERROR: {
          if (m_service.m_currentOnFailCallback) {
            m_service.m_currentOnFailCallback(
                WolkaboutFileDownloader::Error::FILE_SYSTEM_ERROR);
          }
      ...
      m_service.requestPacket(m_service.m_currentPacketIndex,
                              m_service.m_currentPacketSize);
    }
...
void FileDownloadService::DownloadState::download(
    const std::string &, uint_fast64_t, const ByteArray &, const std::string &,
