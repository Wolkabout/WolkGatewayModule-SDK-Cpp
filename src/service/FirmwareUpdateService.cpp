...
FirmwareUpdateService::FirmwareUpdateService(
    ...
    : m_currentFirmwareVersion{firmwareVersion},
      ...
      m_wolkFileDownloader{wolkDownloader}, m_urlFileDownloader{urlDownloader},
      ...
      m_firmwareFile{""}, m_autoInstall{false}, m_commandBuffer{
                                                    new CommandBuffer()} {
...
void FirmwareUpdateService::handleFirmwareUpdateCommand(
...
void FirmwareUpdateService::reportFirmwareUpdateResult() {
  ...
  if (m_currentFirmwareVersion != firmwareVersion) {
    sendResponse(
        FirmwareUpdateResponse{FirmwareUpdateResponse::Status::COMPLETED});
  } else {
    sendResponse(FirmwareUpdateResponse{
        FirmwareUpdateResponse::Status::ERROR,
...
void FirmwareUpdateService::IdleState::handleFirmwareUpdateCommand(
    const FirmwareUpdateCommand &firmwareUpdateCommand) {
  ...
  case FirmwareUpdateCommand::Type::FILE_UPLOAD: {
    ...
    if (name.null() || static_cast<std::string>(name).empty()) {
      m_service.sendResponse(FirmwareUpdateResponse{
          FirmwareUpdateResponse::Status::ERROR,
    ...
    if (size.null() ||
        static_cast<uint_fast64_t>(size) > m_service.m_maximumFirmwareSize ||
    ...
    if (m_service.m_wolkFileDownloader.lock()) {
      ...
      m_service.m_autoInstall =
          !autoInstall.null() && static_cast<bool>(autoInstall);

      const auto byteHash =
          ByteUtils::toByteArray(StringUtils::base64Decode(hash));
...
void FirmwareUpdateService::WolkDownloadState::handleFirmwareUpdateCommand(
  ...
  case FirmwareUpdateCommand::Type::ABORT: {
    ...
    m_service.sendResponse(
        FirmwareUpdateResponse{FirmwareUpdateResponse::Status::ABORTED});
...
void FirmwareUpdateService::ReadyState::handleFirmwareUpdateCommand(
    const FirmwareUpdateCommand &firmwareUpdateCommand) {
  switch (firmwareUpdateCommand.getType()) {
  case FirmwareUpdateCommand::Type::INSTALL: {
    m_service.install();
...
void FirmwareUpdateService::sendResponse(
    const FirmwareUpdateResponse &response) {
  m_outboundDataHandler->addFirmwareUpdateResponse(response);
}

void FirmwareUpdateService::onFirmwareFileDownloadSuccess(
    const std::string &filePath) {
  if (m_currentState == m_wolkDownloadState.get() ||
      m_currentState == m_urlDownloadState.get()) {
    m_currentState = m_readyState.get();
    m_firmwareFile = filePath;

    sendResponse(
        FirmwareUpdateResponse{FirmwareUpdateResponse::Status::FILE_READY});
...
void FirmwareUpdateService::onFirmwareFileDownloadFail(
    WolkaboutFileDownloader::Error errorCode) {
  switch (errorCode) {
...
void FirmwareUpdateService::onFirmwareFileDownloadFail(
    UrlFileDownloader::Error errorCode) {
  switch (errorCode) {
...
void FirmwareUpdateService::downloadFirmware(const std::string &name,
                                             uint_fast64_t size,
                                             const ByteArray &hash) {
  if (auto downloader = m_wolkFileDownloader.lock()) {
    sendResponse(
        FirmwareUpdateResponse{FirmwareUpdateResponse::Status::FILE_TRANSFER});
    ...
    m_executor.reset(new std::thread([=] {
      downloader->download(
          name, size, hash, m_firmwareDownloadDirectory,
...
void FirmwareUpdateService::install() {
  if (auto installer = m_firmwareInstaller.lock()) {
    sendResponse(
        FirmwareUpdateResponse{FirmwareUpdateResponse::Status::INSTALLATION});
    ...
    m_executor.reset(new std::thread([=] {
      if (!FileSystemUtils::createFileWithContent(FIRMWARE_VERSION_FILE,
