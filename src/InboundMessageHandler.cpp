...
InboundMessageHandler::InboundMessageHandler(Device device) : m_device{device}, m_commandBuffer{new CommandBuffer()}
{
    for (const std::string& actuatorReference : m_device.getActuatorReferences())
    {
        ...
        setTopic << ACTUATION_SET_TOPIC_ROOT << m_device.getKey() << "/r/" << actuatorReference;
        m_subscriptionList.emplace_back(setTopic.str());

        std::stringstream getTopic("");
        getTopic << ACTUATION_GET_TOPIC_ROOT << m_device.getKey() << "/r/" << actuatorReference;
        m_subscriptionList.emplace_back(getTopic.str());
    }

    // device registration
    std::stringstream registrationTopic("");

    registrationTopic << REGISTRATION_RESPONSE_ROOT << m_device.getKey();

    m_subscriptionList.emplace_back(registrationTopic.str());

    // firmware update
    std::stringstream topic("");
    topic << FIRMWARE_UPDATE_TOPIC_ROOT << m_device.getKey();
    m_subscriptionList.emplace_back(topic.str());

    // file handling
    std::stringstream binaryTopic("");
    binaryTopic << BINARY_TOPIC_ROOT << m_device.getKey();
...
void InboundMessageHandler::messageReceived(const std::string& topic, const std::string& message)
{
    ...
    if (StringUtils::startsWith(topic, ACTUATION_SET_TOPIC_ROOT))
    {
    ...
    else if (StringUtils::startsWith(topic, ACTUATION_GET_TOPIC_ROOT))
    {
        const size_t referencePosition = topic.find_last_of('/');
        if (referencePosition == std::string::npos)
        {
        ...
        addToCommandBuffer([=]() -> void {
            ...
            {
                m_actuationGetHandler(ActuatorGetCommand(reference));
            }
        });
    }
    else if (StringUtils::startsWith(topic, REGISTRATION_RESPONSE_ROOT))
    {
        auto response = RegistrationProtocol::makeRegistrationResponse(std::make_shared<Message>(topic, message));
        if (response)
        {
            addToCommandBuffer([=] {
                if (m_registrationResponseHandler)
                {
                    m_registrationResponseHandler(response);
                }
            });
        }
    }
...
void InboundMessageHandler::setActuatorSetCommandHandler(std::function<void(ActuatorSetCommand)> handler)
{
    m_actuationSetHandler = handler;
}

void InboundMessageHandler::setActuatorGetCommandHandler(std::function<void(ActuatorGetCommand)> handler)
{
...
void InboundMessageHandler::setRegistrationResponseHandler(
  std::function<void(std::shared_ptr<DeviceRegistrationResponse>)> handler)
{
    m_registrationResponseHandler = handler;
}
