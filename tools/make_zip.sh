...
mkdir -p ./tmp-wgmsc
cd ./tmp-wgmsc || exit
git clone https://github.com/Wolkabout/WolkGatewayModule-SDK-Cpp --recurse-submodules
...
filename="WolkGatewayModule-SDK-Cpp-v$(cat RELEASE_NOTES.txt | grep "**Version" | head -1 | sed -e "s/**Version //" | sed -e "s/\*\*//").zip"
